...
namespace Test
{
    ...
    template <typename T, typename R = void>
    struct is_const_callable_member_operator : std::false_type
    {
    };

    template <typename T>
    struct is_const_callable_member_operator<T, void_t<
                                                    decltype(std::declval<const T &>().operator int())>> : std::true_type
    {
    };
    TEST(MyNumberTest, Step6CheckConstructors)
    {
        ...
        auto val_pos = output.find("val: 12");
        ...
        ASSERT_NE(val_pos, std::string::npos)
            << "Program output should contain 'val: 12'. Actual output:\n"
            << output;
    }
    TEST(MyNumberTest, Step6NotTriviallyDestructable)
    {
        ASSERT_TRUE(!std::is_trivially_destructible_v<local::MyNumber>);
    }
    TEST(MyNumberTest, Step6NotTriviallyCopyConstructible)
    {
        ASSERT_TRUE(!std::is_trivially_copy_constructible_v<local::MyNumber>);
    }
    TEST(MyNumberTest, Step6NotThrowMoveConstructible)
    {
        ASSERT_TRUE(std::is_nothrow_move_constructible_v<local::MyNumber>);
    }
    TEST(MyNumberTest, Step6NotTriviallyCopyAssignable)
    {
        ASSERT_TRUE(!std::is_trivially_copy_assignable_v<local::MyNumber>);
    }
    TEST(MyNumberTest, Step6NotThrowMoveAssignable)
    {
        ASSERT_TRUE(std::is_nothrow_move_assignable_v<local::MyNumber>);
    }
    TEST(MyNumberTest, Step6ConstCallable)
    {
        ASSERT_TRUE(is_const_callable_member_function<local::MyNumber>::value);
    }
    TEST(MyNumberTest, Step6ConstCallableOperator)
    {
        ASSERT_TRUE(is_const_callable_member_operator<local::MyNumber>::value);
    }
    TEST(MyNumberTest, Step6TypeConversion)
    {
        int num = 67;
        local::MyNumber mn{num};
        ASSERT_TRUE(static_cast<int>(mn) == num);
    }
}